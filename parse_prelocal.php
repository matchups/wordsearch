...
function corpusInfo ($table, $option) {
    ...
    if (substr ($key, 0, 6) == 'corpus') {
        ...
        $corpusList [$corpus] = $corpus;
				...
				if ($option == 'W') {
					if ($GLOBALS['corpusObjects'][$corpus] -> getSpecialEntry ($newTable)) {
						$corpusTable [$corpus] = $newTable;
						$useWhere [$corpus] = true;
					} else {
						$corpusTable [$corpus] = $table;
						$useWhere [$corpus] = false;
					}
				}
      }
			...
			if ($matches [2] == '@') { // fake item set in code
				if ($useWhere[$corpus]) {
					...
					$moreSQL .= $GLOBALS['corpusObjects'][$corpus] -> moreSQL ($corpusTable [$corpus], $matches[3], $parm);
				}
			} else {
	...
	foreach ($corpusList as $thisCorpus) {
		if ($nonflags[$thisCorpus]) {
			...
			if (count ($corpusList) == 1  &&  !$alias) {
				...
				$filter = "entry.corpus_id <> $corpus";
				if ($useWhere[$thisCorpus]) {
					$code = $GLOBALS['corpusObjects'][$corpus] -> getSpecialEntry ($temp);
					$moreSQL .= " AND ($filter OR EXISTS (SELECT 1 $code AND $rlike )) ";
				} else {
					$flagClause .= " AND ($filter OR $rlike) ";
				}
			}
		}
	}
